...
//
Процедура ОбновитьПоляФормы(Знач СтруктураДляОбновленияФормы, ДопПараметры = Неопределено) Экспорт
	Перем ОчиститьТаблицу;
	...
	Если НЕ ДопПараметры.Свойство("ОчиститьТаблицу", ОчиститьТаблицу) Тогда
	...
	// Если пришла ошибка то нечего обновлять просто выходим
	...
	Для Каждого Элемент Из СтруктураДляОбновленияФормы Цикл
		...
		ПараметрыОбновления.Вставить("ИмяРеквизита", Элемент.Ключ);
		ПараметрыОбновления.Вставить("ЗначениеРеквизита", НовыйОбъект);
		ПараметрыОбновления.Вставить("ОчиститьТаблицу", ОчиститьТаблицу);
		ПараметрыОбновления.Вставить("Тип", ПараметрыСерилизации.Тип);
		
		ОбновитьПолеФормы(ПараметрыОбновления);
...
&НаСервере
Процедура ОбновитьПолеФормы(ПараметрыОбновления)

	ИмяРеквизита		= ПараметрыОбновления.ИмяРеквизита;
	ЗначениеРеквизита	= ПараметрыОбновления.ЗначениеРеквизита;
	ОчиститьТаблицу		= ПараметрыОбновления.ОчиститьТаблицу;
	Тип					= ПараметрыОбновления.Тип;
	
	// Дерево значений
	Если Тип = "ДеревоЗначений" 
		И ЗначениеРеквизита.Строки.Количество() Тогда
		ЭтаФорма.ЗначениеВРеквизитФормы(ЗначениеРеквизита, ИмяРеквизита);
	ИначеЕсли Тип = "ДеревоЗначений" Тогда
		ЭтаФорма[ИмяРеквизита].ПолучитьЭлементы().Очистить();
	// Таблица значений
	ИначеЕсли Тип = "ТаблицаЗначений"
		И ОчиститьТаблицу Тогда
		ЭтаФорма[ИмяРеквизита].Загрузить(ЗначениеРеквизита);
	ИначеЕсли Тип = "ТаблицаЗначений" Тогда 
		Для каждого ТекущаяСтрока Из ЗначениеРеквизита Цикл
			ЗаполнитьЗначенияСвойств(ЭтаФорма[ИмяРеквизита].Добавить(), ТекущаяСтрока);
		КонецЦикла;
	// Другое
	Иначе
		Возврат;
