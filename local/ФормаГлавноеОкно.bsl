...
Процедура ФильтрОбновитьПанель() Экспорт
	...
	Если Не ЗначениеЗаполнено(Кэш.Текущий.ОписаниеРаздела.Номер) Тогда
		
		Возврат;
		
	КонецЕсли;
	
	ФормаДопФильтра = МодульОбъектаКлиент().НайтиФункциюСеансаОбработки("ОбновитьКонтентПодговитьРаздел", Кэш.Текущий.ОписаниеРаздела.ФормыФильтра);
...
Процедура сбисСохранитьФильтрРаздела(Кэш) Экспорт
	...
	Если ЗначениеЗаполнено(Кэш.Текущий.ОписаниеРаздела.Номер) Тогда
		
		Кэш.Парам.ФильтрыПоРазделам.Вставить(Кэш.Текущий.ОписаниеРаздела.Имя, ТекущийФильтр);
...
Функция сбисВосстановитьФильтр(Кэш, СтруктураФильта) Экспорт
	...
	Если СтруктураФильта.Свойство("ПараметрыТекущегоФильтра") Тогда
