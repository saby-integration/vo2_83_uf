...
Функция ОбновитьКонтент(ЛокальныйКэш) Экспорт
	...
	Если ЗначениеЗаполнено(ГлавноеОкно.КонтрагентНоменклатуры) Тогда
		...
		стрНавигация = Новый Структура("НомерСтраницы,РазмерСтраницы");
		
		стрНавигация.НомерСтраницы	= ЭтаФорма.ВладелецФормы.ФильтрСтраница-1; //На апи отсчет страниц с 0, если на дбф с 1, перенести -1 в форму апи		
		стрНавигация.РазмерСтраницы = ЭтаФорма.ВладелецФормы.ЗаписейНаСтранице1С;
